...
class TrafficSignal:
    ...
    def __init__(self, 
        ...
        self.build_phases()
# getControlledLanes: verilen trafik ışığı tarafından kontrol edilen şeritlerin listesini döndürür.
# getControlledLinks:Returns the links controlled by the traffic light, sorted by the signal index 
# and described by giving the incoming, outgoing, and via lane. 
        self.lanes = list(dict.fromkeys(self.sumo.trafficlight.getControlledLanes(self.id)))  # emove duplicates and keep orderR
    ...
    def set_next_phase(self, new_phase):
        """
        Sets what will be the next green phase and sets yellow phase if the next phase is 
        different than the current
        
        Bir sonraki yeşil fazın ne olacağını ayarlar ve
        sonraki faz mevcut fazdan farklıysa sarı fazı ayarlar

        :param new_phase: (int) Number between [0..num_green_phases] 
        """
        new_phase = int(new_phase)
        if self.green_phase == new_phase or self.time_since_last_phase_change < self.yellow_time + self.min_green:
# Mecut faz değişmediyse veya en son faz değişim süresi min_green(default=10)+yellow_time 
# zamanını geçmediyse simülasyon delta_time(5) kadar oynatılır.
            #self.sumo.trafficlight.setPhase(self.id, self.green_phase) 
            self.sumo.trafficlight.setRedYellowGreenState(self.id, self.all_phases[self.green_phase].state)
            self.next_action_time = self.env.sim_step + self.delta_time
        else:
# Farklı bir yol için yeşil faz ayarlaması yapılır. Sarı aktif hale getirilir.
...
# BU FONKSİYON DEĞİŞTİRİLEREK FARKLI ÖDÜL REWARD FONKSİYONLARI YAZILABİLİR.
...
# lane: verilen trafik ışığı tarafından kontrol edilen şeritlerin sözlüğü (dict)
...
# getLastStepVehicleIDsVerilen: şeritteki son adım için araçların kimliklerini döndürür.
