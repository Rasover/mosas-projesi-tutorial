...
@date    2008-07-10
@version $Id: statisticsElements.py 14425 2013-08-16 20:11:47Z behrisch $
...
class Assign:
    def __init__(self, method, totalVeh, noOfStop, avgNoOfStop, totalTravelTime, totalTravelLength, totalDepartDelay, totalWaitTime, avgTravelTime, avgTravelLength, avgTravelSpeed, avgDepartDelay, avgWaitTime, SDTravelTime, SDLength, SDSpeed, SDWaitTime):
        self.label = method
        self.totalVeh = totalVeh
        self.noOfStop = noOfStop
        self.avgNoOfStop = avgNoOfStop
    ...
    def __repr__(self):
        return "%s_<%s|%s|%s|%s|%s|%s|%s|%s|%s>" % (self.label, self.totalVeh, self.noOfStop, self.avgTravelTime, self.avgTravelLength, self.avgTravelSpeed,
...
class VehInformationReader(handler.ContentHandler):
    ...
    def startElement(self, name, attrs):
        if name == 'tripinfo':
            ...
            self._Vehicle.waitingCount =  float(attrs['waitingCount']) 
...
def getStatisticsOutput(assignments, outputfile):
    ...
    for method in assignments.itervalues():
        ...
        foutveh.write('- SD Travel Time(dl):%s\n' %method.SDTravelTime)
        foutveh.write('- SD Length(dl):%s\n' %method.SDLength)
        foutveh.write('- SD  speed(dl):%s\n' %method.SDSpeed)
        foutveh.write('- SD travel speed(dl):%s\n' %method.SDWaitTime)
    '''
    foutveh.write('<data>\n\t<items>')
    #foutveh.write('average vehicular travel length(m) = the sum of all vehicular travel lengths / the number of vehicles\n')
    for method in assignments.itervalues():
        ...
        foutveh.write('\n\t\t<item name="averageDepartureDelay">%0.2f' %method.avgDepartDelay + '</item>')    
        foutveh.write('\n\t\t<item name="totalWaitingTime">%d' %method.totalWaitTime + '</item>')    
        foutveh.write('\n\t\t<item name="averageVehicularWaitingTime">%0.2f' %method.avgWaitTime + '</item>')
        foutveh.write('\n\t\t<item name="totalNumberOfStop">%d' %method.noOfStop + '</item>')
        foutveh.write('\n\t\t<item name="averageNumberOfStop">%0.2f' %method.avgNoOfStop + '</item>')
        foutveh.write('\n\t\t<item name="totalTravelTime">%d' % method.totalTravelTime + '</item>')    
        foutveh.write('\n\t\t<item name="averageVehicularTravelTime">%0.2f' %method.avgTravelTime + '</item>')
        foutveh.write('\n\t\t<item name="totalTravelLength">%d' %method.totalTravelLength + '</item>')
        foutveh.write('\n\t\t<item name="averageVehicularTravelLength">%0.2f' %method.avgTravelLength + '</item>')
        foutveh.write('\n\t\t<item name="averageVehicularTravelSpeed">%0.2f' %method.avgTravelSpeed + '</item>')
        foutveh.write('\n\t\t<item name="SDTravelTime">%0.2f' %method.SDTravelTime + '</item>')
        foutveh.write('\n\t\t<item name="SDLength">%0.2f' %method.SDLength + '</item>')
        foutveh.write('\n\t\t<item name="SDSpeed">%0.2f' %method.SDSpeed + '</item>')
        foutveh.write('\n\t\t<item name="SDWaitTime">%0.2f' %method.SDWaitTime + '</item>')
    foutveh.write('\n\t</items>\n</data>')
    foutveh.close()
